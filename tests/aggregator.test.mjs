...
test("extractHosts: unique registrable hosts, www stripped, non-http ignored", () => {
  assert.deepEqual(extractHosts(raw), ["chub.ai", "character-tavern.com", "botbooru.com", "newsite.example", "rentry.org"]);
});
test("discover: returns hosts not already in the registry, with provenance", async () => {
  ...
  assert.deepEqual(candidates.map(c => c.host), ["newsite.example", "rentry.org"]);
