...
const IGNORE = new Set(["github.com", "githubusercontent.com", "google.com", "youtube.com", "discord.gg", "discord.com", "reddit.com", "twitter.com", "x.com", "imgur.com", "wikipedia.org"]);

export function extractHosts(text) {
  const out = [];
  for (const m of String(text).matchAll(/https?:\/\/([a-z0-9.-]+\.[a-z]{2,})(?=[/\s"'<>)\]]|$)/gi)) {
    const host = m[1].toLowerCase().replace(/^www\./, "");
    if (!IGNORE.has(host) && !out.includes(host)) out.push(host);
  }
  return out;
}
